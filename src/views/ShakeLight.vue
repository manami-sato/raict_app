<template lang="pug">
main(:style="{minHeight:height+`px`, background:`linear-gradient(150deg,` + bgColor1 + ` 0%, ` + bgColor2 + ` 100%)`}").ShakeLight
	div(v-if="loadFlag").ShakeLight__load
		p.ShakeLight__load--txt ライブ開始までお待ちください。
		svg(version="1.1",xmlns="http://www.w3.org/2000/svg",xmlns:xlink="http://www.w3.org/1999/xlink",x="0px",y="0px",viewBox="0 0 512 512",xml:space="preserve").ShakeLight__load--icon
			g
	...
	div(v-if="!loadFlag").ShakeLight__shake
		div(:class="[{shake1:count == 1},{shake2:count == 2},{shake3:count == 3},{shake4:count == 4},{shake5:count == 5},{shake6:count == 6},{shake6:count == 6},{shake7:count == 7},{shake8:count == 8}]").ShakeLight__shake--circle
	div(v-if="!loadFlag").ShakeLight__swipe
		div(v-if="announceFlag",:class="{announceActive:announceFlag}").ShakeLight__swipe--nav
			|上にスワイプで<br>
			|ライブモードに切り替え！
		div(@click="toLiveMode").ShakeLight__swipe--tab
...
export default {
  ...
  methods: {
    ...
    toLiveMode() {
      this.$router.push({
        name: "WatchLive",
        path: `${this.routerPath}movie/:colorId`,
        params: {
          colorId: `${this.id}`,
        },
        props: true,
  ...
  mounted() {
    ...
    setTimeout(announceDisplay, 4700);
